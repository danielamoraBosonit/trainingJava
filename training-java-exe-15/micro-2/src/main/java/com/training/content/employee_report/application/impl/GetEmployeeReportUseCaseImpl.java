...
public class GetEmployeeReportUseCaseImpl implements GetEmployeeReportUseCase {
    ...
    @Override
    public EmployeeReport getEmployeeReport(String email, Integer month, Integer year) {
        ...
        List<TimeClock> timeClockList = getTimeClockUseCase
                .getAllByEmailAndTimeBetween(email, from, to);

        return buildEmployeeReport(email, timeClockList, yearMonth);
    }

    private EmployeeReport buildEmployeeReport(String email, List<TimeClock> timeClockList, YearMonth yearMonth) {
    ...
    private Duration calculateDayTime(List<TimeClock> timeClockDayList) {
        ...
        for (int i = 0; i < timeClockDayList.size(); i++){
            ...
            if (i % 2 == 0){
                if (!timeClockDayList.get(i).getType().toUpperCase().equals(TimeClockType.ENTER.name())) return null;
                startTime = timeClockDayList.get(i).getTime();

            } else {
                if (!timeClockDayList.get(i).getType().toUpperCase().equals(TimeClockType.EXIT.name()) ||
